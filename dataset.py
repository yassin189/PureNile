...
def Get_db_dates():
    ...
    #loop through all database names
    for x in range(0, dbnamesLength):
        if("iotp_j1kovt_all" in dbnames[x]) and ("-" in dbnames[x]):
            #get the date from the database name
            dbNameSplit = dbnames[x].split("_")
            dbDate = dbNameSplit[5]
            ...
            if (uniqueDatesExists == False) and ("-" in dbDate):
                ...
                for y in range(0, rowsLength):
                    if hasLocation == False:
                        ...
                        if("deviceId" in data["rows"][y]["doc"])and ("Authority" in data["rows"][y]["doc"]["data"]) and ((data["rows"][y]["doc"]["data"]["Authority"]) == Authority):
                            hasLocation = True
...
def Get_db_deviceids():
    """
    Collect and returns device ids from the Cloudant storage
    """
    #connect to Cloudant
    cloudantClient.connect()
    ...
    #loop through all database names
    for x in range(0, dbnamesLength):
        if("iotp_j1kovt_all" in dbnames[x]) and ("-" in dbnames[x]): #and count < 11
